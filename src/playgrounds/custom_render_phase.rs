...
impl Plugin for CustomRenderPhasePlugin {
    fn build(&self, app: &mut App) {
        app.add_plugins(MeshStencilPhasePlugin);
        app.add_systems(
            OnEnter(PlaygroundScene::CustomRenderPhase),
...
fn setup(mut commands: Commands) {
    let scene = bsn! {
        Visibility::Visible
        Transform
        DespawnOnExit<PlaygroundScene>(PlaygroundScene::CustomRenderPhase)
        Children [
            // circular base
...
pub struct MeshStencilPhasePlugin;
